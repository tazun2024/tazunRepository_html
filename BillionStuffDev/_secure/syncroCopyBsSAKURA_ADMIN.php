<?php
    ...
    include(APP_ROOT.'COMMON/Cfg/InvestxCfg.php');
            ...
            $myCmxLog = new CmxLog(__DIR__.'/syncroCopyBS_log/'.date('YmdHis').'.log');
            $myCmxLog->infEcho('>>execute syncroCopyBsSAKURA_ADMIN');

            $myBSDao = new BSDao($myCmxLog, INVESTX_NOT_APPLICABLE_VALUE);

            //////// クエリの実行
            $myBSDao->getListTypeRecord("delete from BS_ADMIN where meigara_cd = 'CR_N225M'");


            //////// //////// //////// ////////
            $url = 'https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/View/tmp.selectResultBs.php?execQuery='.str_replace(' ', '%20', "select meigara_cd,date,side,value,price from INVESTX_ADMIN where meigara_cd='CR_N225M'&dTimeColumn=date&");
            //////// //////// //////// ////////
...
****/
                ...
                if (isset($node->childNodes[0])) {
                    ...
                    if ($node->childNodes[0]->nodeValue == 'CR_N225M') {

                        $SQL = "insert into BS_ADMIN values('".$node->childNodes[0]->nodeValue.        // meigara_cd
                        ...
                        //////// クエリの実行
                        $myBSDao->getListTypeRecord($SQL);


                        //////// 現行は建玉を1世代しか持っていないので世代ロ―テートの時に失敗してしまう、暫定対応（同じものをinsert）
                        if ($side == 'tateGYK1') {
            ...
            $myBSDao->getListTypeRecord("update BS_ADMIN set value = '".$NY_ELMCT."' where meigara_cd = 'CR_N225M' and side = 'NY_ELMCT';");

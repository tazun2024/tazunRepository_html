<?php
    ...
    include(APP_ROOT.APP_TYPE.'/Util/BSUtil.php'); // FSDao内で BSUtil::T_CMX_SIMULATIONの場合分けをするので必要
    ...
    include(APP_ROOT.'COMMON/Cfg/InvestxCfg.php');
            ...
            $myBSDao = new BSDao($myCmxLog, INVESTX_NOT_APPLICABLE_VALUE, CoreBase::T_AUD5M);

            //////// クエリの実行
            $deleteQuery= "delete from BS_ADMIN  where meigara_cd = 'CR_N225M'";
            echo '$deleteQuery=['.$deleteQuery.']<br><br>';
            $myBSDao->getListTypeRecord($deleteQuery);


            //////// //////// //////// ////////
            $baseUrl = 'https://cmx.boy.jp/html/BillionStuffRsv/CmxApi/queryBS.php';
            $queryStr = "select meigara_cd, date, side, value, price from BS_ADMIN where meigara_cd='CR_N225M'";
            $url = $baseUrl.'?logId=syncroCopyBsLOLOPOP_ADMIN'.'&QUERY='.urlencode($queryStr);
            //////// //////// //////// ////////
...
//echo $result;
            ...
            foreach($rowNodes as $theRow){
                ...
                if ($meigaraCd == 'CR_N225M') {

                    $SQL = "insert into BS_ADMIN values('CR_N225M".       // meigara_cd
        ...
        ////$myBSDao->getListTypeRecord("update FS_ADMIN set value = '".$NY_ELMCT."' where meigara_cd = 'CR_N225M' and side = 'NY_ELMCT';");

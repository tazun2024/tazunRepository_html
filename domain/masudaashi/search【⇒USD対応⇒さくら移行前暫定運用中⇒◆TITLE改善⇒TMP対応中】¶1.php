...
////include(APP_ROOT.'COMMON/Util/NewSimpleUtil.php');             //// search_Common.phpで定義される
  ...
  echo "<HEAD>\n";
  echo "<TITLE>MY_MA SEARCH {$targetCode}</TITLE>\n";
  ...
  echo "<BODY>\n";
      ...
      //$viewMsgArr[] = '指定された日付['.$_REQUEST['TARGET_YMD'].']';
        // 2022.10.17 確かに消える「Notice: Undefined index: TARGET_YMD in /home/users/2/boy.jp-cmx/web/html/domain/masudaashi/search.php on line 62」
        $viewMsgArr[] = '指定された日付['.@$_REQUEST['TARGET_YMD'].']';
        ...
        //echo 'さくらAPIの営業日判定結果['.$chkYmd.']<br>';
          $viewMsgArr[] = 'さくらAPIの営業日判定結果['.$chkYmd.']';
        ...
        //echo 'rev['.$prvfwdResult->rev.']<br>';
          $viewMsgArr[] = 'rev['.$prvfwdResult->rev.']';
        //echo 'fwd['.$prvfwdResult->fwd.']<br>';
          $viewMsgArr[] = 'fwd['.$prvfwdResult->fwd.']';
  ...
  $bachuTmpRequest = @$_REQUEST['BACHU_TMP_REQUEST'] ?: '';
      ...
      if ($targetCode <> '') {
        ...
        if ($targetYmd == '') {
          ...
          // ==== さくらサーバに一時ファイルを作成させる
          $dummyResult = file_get_contents('http://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI.php?TARGET_CODE='.$targetCode.'&BACHU_TMP_REQUEST='.$bachuTmpRequest);
          ...
          if (mb_strpos($dummyResult, 'no_img') === false) {
            ...
            file_put_contents(DOMAIN_ROOT.'masudaashi/_img/'.$targetCode.$targetFile, $image);
              ...
              // ==== 属性情報ファイルを取得
              $attribute = file_get_contents("http://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/_fileWork/".$targetCode."target.gif.attribute?dummy=".date('Ymdhis'));
              file_put_contents(DOMAIN_ROOT.'masudaashi/_img/'.$targetCode.$targetFile.'.attribute', $attribute);
              include(DOMAIN_ROOT.'masudaashi/_img/'.$targetCode.$targetFile.'.attribute');
          ...
          if ( false
           ...
           and strtotime($targetYmd) <= strtotime(ORGMASUDAASHI_LEASTDAY)
          ...
          //$dummyResult = file_get_contents('http://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI.php?TARGET_CODE='.$targetCode.'&TARGET_DATE='.$targetYmd.'&BACHU_TMP_REQUEST='.$bachuTmp);
            $dummyResult = file_get_contents('http://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI.php?TARGET_CODE='.$targetCode.'&TARGET_DATE='.$targetYmd); // 日付指定「あり」では、BACHU_TMP_REQUESTも「あり」はありえない
        ...
        echo "<input onfocus='this.select();' type='text' size='20' name='TARGET_CD' value='".$targetCode."'>\n";
       ...
       //and $bachuTmpR <> 'DONE' ) {                              // 場中tmpがある状態で、その表示をリクエストする[fwd]が押された場合の遷移でセットされてくる（&BACHU_TMP=DONE）
         and $bachuTmpRequest <> 'DONE' ) {                         // 場中tmpがある状態で、その表示をリクエストする[fwd]が押された場合の遷移でセットされてくる（&BACHU_TMP=DONE）
      ...
      //echo '<p>場中tmpアルヨ！'.mb_substr($dummyResult, mb_strpos($dummyResult, 'BACHU_TMP=EXIST') + 15, 10)."</p>\n";
        ...
        ////echo "<font size=+1>&nbsp;<br><a href=http://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI.php?TARGET_CODE=".$targetCode."&BACHU_TMP_REQUEST=DONE>.》TMP</a></font>";
            if ($bachuTmpRequest == '') echo "<font size=+1>&nbsp;<br><a href=./search.php?TARGET_CD=".$targetCode."&BACHU_TMP_REQUEST=DONE>.》TMP</a></font>";
            echo "</td>\n";
          }
      ...
      //$meigaraNm = getMeigaraNm($targetCode);
        ...
        if ($bachuTmpRequest == '') { // ¶BACHU_TMP_REQUESTも「あり」の場合は6色分布表示選択はできない

            if ($sixclrDistribution == '' and $bachuTmpRequest <> '') { // ¶BACHU_TMP_REQUESTも「あり」の場合は6色分布表示選択はできない
                ...
                if (!isset($_REQUEST['TARGET_YMD'])) {
                    ...
                    if ($sixclrDistribution == 'N225') {
                      ...
                      //$dummyResult = file_get_contents("http://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI_SIXCLR_MOTHERS.php");
                        // 2022.10.17 確かに消える「Warning: file_get_contents～: failed to open stream: HTTP request failed! HTTP/1.1 404 Not Found in /home/users/2/boy.jp-cmx/web/html/domain/masudaashi/search.php on line 474」
                        $dummyResult = @file_get_contents("http://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI_SIXCLR_MOTHERS.php");
      ...
      echo "</tr>";
          ...
          /* ---- 2022.09.30 ---- */
          /* 新しいタブ */
          echo '<tr><td colspan=2 style="word-break:break-all; font-size:small;">';
          echo '<style type=text/css>';
          echo '.cmxTabWrap {display:flex; flex-wrap: wrap;}';
          echo '.cmxTab {font-size:small; font-weight:bold; color:White; background:LightGray; margin-right:5px; padding:3px 12px; order:-1;}';
          echo '.cmxTab-content {width:100%; display:none;}';
          echo '.cmxTabSwitch:checked+.cmxTab+.cmxTab-content {display:block;}';
          echo '.cmxTabSwitch:checked+.cmxTab {color:Black; background: #D2D2FF;}';
          echo '.cmxTabSwitch {display:none;}';
          echo '</style>';

          echo '<div class=cmxTabWrap>';
          echo '<input id=TAB-OFF type=radio name=TAB value=1 class=cmxTabSwitch checked=checked />';
          echo '<label class=cmxTab for=TAB-OFF>ログステータス非表示</label>';
          echo '<div class=cmxTab-content>';
          echo '</div>';
          echo '<input id=TAB-ON type=radio name=TAB value=2 class=cmxTabSwitch  />';
          echo '<label class=cmxTab for=TAB-ON>表示</label>';
          echo '<div class=cmxTab-content>';
          foreach ($viewMsgAr as $msg) {

...
//echo 'さくらAPIの営業日判定結果['.$chkYmd.']<br>';
  ...
  echo "<BODY>\n";
      ...
      echo '<hr>';
      echo '<a href=./search.php?TARGET_CD=7342'.$sixClrDistributionLinkStr.'>7342 ウェルスナビ(株)</a><br>';
      echo '<a href=./search.php?TARGET_CD=9828'.$sixClrDistributionLinkStr.'>9828 元気寿司(株)</a><br>';
      echo '<a href=./search.php?TARGET_CD=4540'.$sixClrDistributionLinkStr.'>4540 (株)ツムラ</a><br>';
      echo '<a href=./search.php?TARGET_CD=8252'.$sixClrDistributionLinkStr.'>8252 (株)丸井グループ</a><br>';
      echo '<a href=./search.php?TARGET_CD=9468'.$sixClrDistributionLinkStr.'>9468 (株)ＫＡＤＯＫＡＷＡ</a><br>';
      echo '<hr>';
      try {
  ...
  $bachuTmp = @$_REQUEST['BACHU_TMP'] ?: '';
      ...
      if ($targetCode <> '') {
        ...
        if ($targetYmd == '') {
          ...
          // ==== さくらサーバに一時ファイルを作成させる
          $dummyResult = file_get_contents('https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI.php?TARGET_CODE='.$targetCode.'&BACHU_TMP_REQUEST='.$bachuTmp);

          if (mb_strpos($dummyResult, 'no_img') === false) {

            // ==== 画像ファイルを取得
            $image = file_get_contents("https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/_fileWork/".$targetCode."target.gif?dummy=".date('Ymdhis'));
          ...
          if ( strtotime($targetYmd) <= strtotime(ORGMASUDAASHI_LEASTDAY)
           and file_exists($WORKFILE_DIR.$targetCode.$targetFile) ) {
            ...
            // ==== さくらサーバに一時ファイルを作成させる
            $dummyResult = file_get_contents('https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI.php?TARGET_CODE='.$targetCode.'&TARGET_DATE='.$targetYmd.'&BACHU_TMP_REQUEST='.$bachuTmp);
        ...
        if (mb_strpos($dummyResult, 'no_img') === false) {
            ...
            switch($targetCode) {
                ...
                case '1001': // 日経２２５種平均
                case '1009': // 東証マザーズ指数
                case '1021': // ミニ２２５先物（中心限月）
                case '1003': // 米ドル／円
                case '1052': // 豪ドル／円
                case '1081': // ダウ工業株３０種平均
                    break;
        ...
        if (mb_strpos($dummyResult, 'MY_MA_5M=EXIST') !== false) {
          echo "<A href='https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI_5M.php?TARGET_CODE=".$targetCode."'>[5分足 ".$targetCode."]</A>\n";
        }
        ...
        if ( false and // 暫定運用中は場中TMP表示は見送り
             mb_strpos($dummyResult, 'BACHU_TMP=EXIST') !== false // 画像を生成してみて、「場中tmpが存在している」と情報が返却された
      ...
      //$meigaraNm = getMeigaraNm($targetCode);
        ...
        echo     "&nbsp;<input type='text' size='6' name='TARGET_YMD' value='".$targetYmd."' onfocus='this.select();'>";
        echo     "&nbsp;<input type='submit' value='日付指定'>";
        echo     "</TD>";
        echo     "<TD>";
        echo     "&nbsp;<a href=https://cmx.boy.jp/html/domain/marketInfo/marketInfo.php?TARGET_CODE=".$targetCode."><img width=48 src=./_img/linkMarketInfo.png></a>";
        echo     "&nbsp;<a href=https://cmx.boy.jp/html/BillionStuffRevolutionStg/View/indexBSR.php?MEIGARA_CD=".$targetCode."><img width=48 src=./_img/linkBillionstuffRevolution.png></a>";
        echo     "&nbsp;<a href=https://finance.yahoo.co.jp/quote/".$targetCode."/chart><img width=48 src=./_img/linkYahoo.png></a>";
        if ( $targetCode <> '1001' and
             $targetCode <> '1009' and
             $targetCode <> '1021' and
             $targetCode <> '1003' and
             $targetCode <> '1052' and
             $targetCode <> '1081'     ) {
            echo     "&nbsp;<a href=https://kabuyoho.jp/sp/reportTarget?bcode=".$targetCode." onclick=\"window.open('https://kabuyoho.jp/sp/reportTarget?bcode=".$targetCode."', 'kabupro','width=640,height=480,noopener'); return false;\"><img width=48 src=./_img/linkKabupro.png></a>";
        } else {
            echo     "&nbsp;";
        }
        echo     "</TD>";
        ...
        if ($sixclrDistribution == '') {
          ...
          if (!isset($_REQUEST['TARGET_YMD'])) {
            ...
            if ($sixclrDistribution == 'N225') {
              $dummyResult = file_get_contents("https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI_SIXCLR.php");
              $image = file_get_contents("https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/_fileWork/sixClrDistributiontarget.gif"."?dummy=".date('Ymdhis'));
              file_put_contents(DOMAIN_ROOT.'masudaashi/_img/sixClrDistributiontarget.gif', $image);

            //echo "<img src='https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/_fileWork/sixClrDistributiontarget.gif'><BR>\n";
              echo "<img src='https://cmx.boy.jp/html/domain/masudaashi/_img/sixClrDistributiontarget.gif'><BR>\n";
              echo "<a href='./search.php?TARGET_CD=".$targetCode."'>6色分分布図非表示</a>";
            //echo "&nbsp;<a href='./search.php?SIXCLR_DISTRIBUTION=MOTHERS&TARGET_CD=".$targetCode."'>（マザーズの6色分布図を表示）</a><BR>\n";
            } else {
              $dummyResult = file_get_contents("https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI_SIXCLR_MOTHERS.php");
              $image = file_get_contents("https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/_fileWork/sixClrDistributionMotherstarget.gif"."?dummy=".date('Ymdhis'));
              file_put_contents(DOMAIN_ROOT.'masudaashi/_img/sixClrDistributionMotherstarget.gif', $image);

            //echo "<img src='https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/_fileWork/sixClrDistributionMotherstarget.gif'><BR>\n";
              echo "<img src='https://cmx.boy.jp/html/domain/masudaashi/_img/sixClrDistributionMotherstarget.gif'><BR>\n";
            ...
            if ($sixclrDistribution == 'N225') {
              $dummyResult = file_get_contents("https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI_SIXCLR.php?TARGET_DATE=".$ymd);
              $image = file_get_contents("https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/_fileWork/sixClrDistributiontarget.gif"."?dummy=".date('Ymdhis'));
              file_put_contents(DOMAIN_ROOT.'masudaashi/_img/sixClrDistributiontarget.gif', $image);
            ...
            //echo "<a href='./search.php?TARGET_CD=".$targetCode."&TARGET_YMD=".$ymd."'>6色分布図非表示</a>";
              ...
              $dummyResult = file_get_contents("https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/MY_MASUDAASHI_SIXCLR_MOTHERS.php?TARGET_DATE=".$ymd);
              $image = file_get_contents("https://ik1-326-23246.vs.sakura.ne.jp/BillionStuff/%5Eact/MY_MASUDAASHI/_fileWork/sixClrDistributionMotherstarget.gif"."?dummy=".date('Ymdhis'));
              file_put_contents(DOMAIN_ROOT.'masudaashi/_img/sixClrDistributionMotherstarget.gif', $image);

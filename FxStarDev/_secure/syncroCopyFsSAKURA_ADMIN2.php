<?php
    ...
    include(APP_ROOT.'COMMON/Cfg/InvestxCfg.php');
            ...
            //////// クエリの実行
            $deleteQuery= "delete from FS_ADMIN where meigara_cd = 'CR_FSAUD' and side not like 'HBAR%' and side not in ('WV_BTM', 'WV_BTM_L', 'WV_TOP', 'WV_TOP_L', 'SARA_N', 'SARA_P')";
            echo '$deleteQuery=['.$deleteQuery.']<br><br>';
            $myFSDao->getListTypeRecord($deleteQuery);


            //////// //////// //////// ////////
            $url = 'https://ik1-326-23246.vs.sakura.ne.jp/FxStar/%5Eact/View/tmp.selectResultFs.php?execQuery='.str_replace(' ', '%20', "select meigara_cd,date,side,value,price from FS_ADMIN where meigara_cd='CR_FX'&dTimeColumn=date&");
            //////// //////// //////// ////////
...
****/
                ...
                ////if ($side === 'tateGYKU') $side = 'tateGYK1';
                    ...
                    if ($node->childNodes[0]->nodeValue == 'CR_FX') {

                        if ( $side <> 'LST_NEL'  and $side <> 'LST_PEL'
                         and $side <> 'SARA_N'   and $side <> 'SARA_P'
                         and $side <> 'WV_TOP'   and $side <> 'WV_BTM'
                         and $side <> 'WV_TOP_L' and $side <> 'WV_BTM_L' ) { // 暫定実装の済んだsideを除外してシンクロ
